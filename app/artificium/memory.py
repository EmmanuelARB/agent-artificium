...
# Real/estimate token calibration (B3) lives in calibration.py; re-exported
# here (and used below by WorkingMemory.estimated_tokens) so both a workspace
# overlay of just this module and any code still written against the
# pre-split layout (`from .memory import CalibrationState`) keep working.
...
class WorkingMemory:
    def __init__(
        ...
        # (B10) Avoid re-reading and re-parsing working_context.jsonl on every
        # call within the same round; a real edit always changes mtime/size.
        self._load_cache_key: tuple[int, int] | None = None
        self._load_cache_value: list[dict[str, Any]] = []

    def load(self) -> list[dict[str, Any]]:
        try:
            stat = self.paths.working_context.stat()
            key: tuple[int, int] | None = (stat.st_mtime_ns, stat.st_size)
        except OSError:
            key = None
        if key is not None and key == self._load_cache_key:
            # Deep-copy out so a caller mutating its result cannot corrupt the cache.
            return copy.deepcopy(self._load_cache_value)
        value = read_jsonl(self.paths.working_context)
        if key is not None:
            self._load_cache_key = key
            self._load_cache_value = copy.deepcopy(value)
        return value
    ...
    def estimated_tokens(self, extra_tokens: int = 0) -> int:
        raw = self.estimator.messages(self.load()) + max(0, int(extra_tokens))
        # (B3) Benefit context-pressure/offload callers from the same real-usage
        # calibration as the request-measurement path, once any data exists.
        calibration = load_calibration(self.paths, self.config.provider, self.config.model)
        if calibration.samples > 0:
            return max(1, round(raw * calibration.ratio))
        return raw
        ...
        *,
        ...
        if mode == "overwrite" and exists:
            check_shrink_guard(
                ...
                new_size=len(rendered.encode("utf-8")),

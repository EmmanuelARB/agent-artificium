...
# Defaults a fresh setup writes. Dataclass defaults above stay as they were so
# a saved configuration, a library caller, or an older file keeps its meaning.
NEW_SETUP_DEFAULTS: dict[str, Any] = {
    "request_timeout_seconds": 1800.0,
    "mandatory_offload": True,
    "offload_threshold_percent": 80.0,
    "auto_repair": True,
    "working_memory_fraction": 0.6,
    "chars_per_token": 3.2,
...
MODEL_FIELDS = frozenset({
    ...
    "vision", "model_supports_vision", "prompt_cache", "prompt_cache_key",
    "stream_responses", "stall_timeout_seconds", "first_token_timeout_seconds",
    *GENERATION_FIELDS,
})
...
def parse_stall_timeout(value: float | str | None) -> float | None:
    """None/off disables the streaming inactivity watchdog entirely, leaving
    only the overall request timeout as a bound. Unlike the whole-request
    timeout, a fraction of a second is a legitimate, deliberately tight
    value here (and is useful for exercising the watchdog in tests)."""
    if value is None or (isinstance(value, str) and value.strip().lower() == "off"):
        return None
    message = "Stall timeout must be off or between 0.1 and 86,400 seconds"
    try:
        seconds = float(value)
    except (TypeError, ValueError):
        raise ValueError(message) from None
    if isinstance(value, bool) or not 0.1 <= seconds <= 86_400:
        raise ValueError(message)
    return seconds


def parse_first_token_timeout(value: float | str | None) -> float | None:
    """None/off (the default) waits for the full request timeout before the
    first streamed chunk; a number bounds prefill silence more tightly."""
    if value is None or (isinstance(value, str) and value.strip().lower() == "off"):
        return None
    message = "First-token timeout must be off or between 0.1 and 86,400 seconds"
    try:
        seconds = float(value)
    except (TypeError, ValueError):
        raise ValueError(message) from None
...
class Config:
    ...
    # Streaming inactivity watchdog: once the first streamed chunk (content,
    # reasoning, or a keep-alive) has arrived, abort if the gap to the next
    # one exceeds this. None/off leaves only request_timeout_seconds.
    stall_timeout_seconds: float | None = 180.0
    # Separate, more generous allowance for prefill (before any chunk has
    # arrived): prompt processing can legitimately take many minutes. None
    # (the default) means bounded only by request_timeout_seconds.
    ...
    # Share of the model context used as the offloading target when no exact
    # token target is set. None with no token target means the whole context.
    working_memory_fraction: float | None = None
    ...
    # Ask providers that need explicit markers (Anthropic) to cache the stable
    # prompt prefix. Providers that cache automatically ignore it.
    prompt_cache: bool = True
    # Optional routing hint for OpenAI-style prompt caches; "auto" derives one
    # from instance_id. Off by default: some compatible servers reject it.
    prompt_cache_key: str | None = None
    # Stream chat-completions replies so progress is visible while generating.
    stream_responses: bool = False
    # Keep Self and meta-memory frozen in the system prompt between context
    # rebuilds and report later edits as runtime notices, so an edit does not
    # invalidate the provider's cache for the whole working context.
    pinned_mind_snapshot: bool = True
    ...
    def __post_init__(self) -> None:
        ...
        self.stall_timeout_seconds = parse_stall_timeout(self.stall_timeout_seconds)
        self.first_token_timeout_seconds = parse_first_token_timeout(self.first_token_timeout_seconds)
        ...
        if self.working_memory_fraction is not None and (
            isinstance(self.working_memory_fraction, bool)
            or not isinstance(self.working_memory_fraction, (int, float))
            or not 0.1 <= self.working_memory_fraction <= 1.0
        ):
            raise ValueError("working_memory_fraction must be null or between 0.1 and 1.0")
        for name in ("prompt_cache", "stream_responses", "pinned_mind_snapshot"):
            if not isinstance(getattr(self, name), bool):
                raise ValueError(f"{name} must be boolean")
        if self.prompt_cache_key is not None and (
            not isinstance(self.prompt_cache_key, str) or not self.prompt_cache_key.strip()
        ):
            raise ValueError("prompt_cache_key must be null, auto, or a non-empty string")
    ...
    def working_memory_limit(self) -> int:
        ...
        if self.working_memory_tokens:
            return self.working_memory_tokens
        if self.working_memory_fraction:
            return min(
                self.context_window_tokens,
                max(4_000, int(self.context_window_tokens * self.working_memory_fraction)),
            )
...
class ConfigStore:
    ...
    def save(self, config: Config) -> None:
        self.paths.install.mkdir(parents=True, exist_ok=True)
...
def configured_paths(start: str | Path | None = None) -> Paths:
    """Resolve the project root: an explicit choice (``--root``, used by the
    background launcher and tests), then ``ARTIFICIUM_ROOT``, then the clone
    that contains the running ``app/`` tree."""

    if start is not None:
        return Paths(Path(start).expanduser().resolve())
    override = os.getenv("ARTIFICIUM_ROOT")
    if override:
        return Paths(Path(override).expanduser().resolve())
    # _bootstrap always loads from app/, even when this module is a workspace
    # override living under workspace/overrides/code/.
    return Paths.from_code_file(_bootstrap.__file__)

...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--root",
        help="Installation root containing config.json, app/, and workspace/",
    )
    parser.add_argument(
        "--no-overrides",
        action="store_true",
    ...
    reset = commands.add_parser(
        "reset",
        help="Archive the workspace and grow a factory-fresh mind from the seed",
    )
    reset.add_argument("--yes", action="store_true", help="Do not ask for confirmation")
    reset.add_argument(
        "--discard",
        action="store_true",
        help="Delete the workspace instead of archiving it",
    )
    ...
    upgrade = commands.add_parser("upgrade", help="Upgrade the application under app/; the workspace is untouched")
...
def _reset(paths: Paths, args: argparse.Namespace) -> int:
    ...
    existed = paths.root.is_dir()
    if existed:
        fate = "delete" if args.discard else "archive"
        if not _confirm(
            f"This will {fate} everything the agent has learned in {paths.root}. "
            f"Settings in {paths.config.name} are kept. Continue?",
            assume_yes=args.yes,
        ):
        ...
        if args.discard:
            ...
            archive = paths.install / "archives" / sortable_id("workspace_")
            archive.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(paths.root), str(archive))
            print(f"Archived the previous workspace: {archive}")
